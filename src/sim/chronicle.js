...
import { clamp, gini, mean, topN } from '../core/util.js';
...
export class Chronicle {
  constructor(lang) {
    this.lang = lang;
    this.events = [];              // every event, forever
    this.series = [];              // periodic snapshots
    this.eras = [{ index: 0, name: `${lang.coinRaw(2)}, ${ERA_NAMES[0]}`, startTick: 0, capability: 0, endTick: null }];
    this.landmarks = [];
    this.lastReportTick = 0;
    this.reportCount = 0;
    this.byAgent = new Map();
    this.causes = new Map();
    this.priceHistory = new Map();  // conceptKey -> [{tick, price}]
  }
  ...
  forAgent(id) { return this.byAgent.get(id) || []; }

  get era() { return this.eras[this.eras.length - 1]; }
  ...
  maybeAdvanceEra(sim) {
    ...
    if (level > cur.index && longEnough && this.eras.length < ERA_NAMES.length) {
      ...
      this.eras.push(era);
      sim.record(null, 'era', `A new age begins — ${era.name}`, { valence: 0.5, intensity: 1, landmark: true });
      return era;
    }
    return null;
  }
  ...
  noteCause(cause) { this.causes.set(cause, (this.causes.get(cause) || 0) + 1); }

  since(tick) { return this.events.filter((e) => e.tick > tick); }
  ...
  report(sim) {
    ...
    const invNew = sim.ont.inventions.filter((i) => (sim.inventionTicks.get(i.key) ?? 0) > from);
    ...
    const r = {
      ...
      knowledge: {
        ...
        breadth: +sim.ont.breadth().toFixed(3),
        bestFor: [...sim.ont.bestFor.entries()].map(([fn, v]) => ({ fn, word: sim.ont.get(v.key)?.word || v.key, score: +v.score.toFixed(2) })).sort((a, b) => b.score - a.score),
        newInventions: invNew.slice(-24).reverse(),
        lostKnowledge: sim.lostKnowledge.slice(-10),
      ...
      society: {
        ...
        leaders: sim.leaders().map((a) => ({ name: a.name, respect: +sim.respectFor(a).toFixed(2), role: a.role })),
      ...
      deaths: dead.slice(-14).reverse().map((a) => ({
        name: a.name, cause: a.deathCause, age: +a.ageAt(a.deathTick).toFixed(1),
        day: Math.floor(a.deathTick / 24) + 1, buried: !!a.buried, mourners: a.mourners || 0,
        legacy: a.stats.inventions + a.stats.taught, children: a.children.length,
      })),
      causes: [...this.causes.entries()].sort((a, b) => b[1] - a[1]),
      landmarks: this.landmarks.slice(-30).reverse(),
      recent: window.slice(-140).reverse(),
      series: this.series,
    };
  ...
  demography(sim) {
    ...
    return {
      ageBuckets: { child: buckets[0], young: buckets[1], adult: buckets[2], mature: buckets[3], elder: buckets[4] },
      meanAge: +mean(living, (a) => a.ageAt(sim.world.tick)).toFixed(1),
      meanLifespan: sim.dead.length ? +mean(sim.dead, (a) => a.ageAt(a.deathTick)).toFixed(1) : null,
      pairs: living.filter((a) => a.partner).length / 2,
      pregnant: living.filter((a) => a.body.pregnant).length,
      orphans: living.filter((a) => a.parents.length && a.parents.every((p) => !sim.byId(p)?.alive)).length,
  ...
  economy(sim) {
    const goods = new Map();
    for (const a of sim.living) for (const [k, v] of a.inventory) goods.set(k, (goods.get(k) || 0) + v);
    const prices = [...sim.marketPrices.entries()].map(([k, v]) => ({
      key: k, word: sim.ont.get(k)?.word || k, price: +v.price.toFixed(2), volume: v.volume,
    })).sort((a, b) => b.volume - a.volume).slice(0, 14);
    return {
      inequality: +gini(sim.living.map((a) => a.carried())).toFixed(3),
      totalGoods: [...goods.values()].reduce((a, b) => a + b, 0),
      topGoods: [...goods.entries()].map(([k, v]) => ({ word: sim.ont.get(k)?.word || k, qty: Math.round(v) })).sort((a, b) => b.qty - a.qty).slice(0, 12),
      prices,
      stores: sim.world.structuresOfKind('store').map((s) => ({ word: s.word, stock: Math.round([...s.stock.values()].reduce((a, b) => a + b, 0)) })),
  ...
  roleCensus(living) {
    ...
    return [...m.entries()].map(([role, n]) => ({ role, n })).sort((a, b) => b.n - a.n);
  }
  ...
  bonds(sim) {
    ...
    for (const a of sim.living) {
      for (const r of a.relationships.values()) {
        ...
        if (strength > 1.1) out.push({ a: a.name, b: o.name, affection: +r.affection.toFixed(2), trust: +r.trust.toFixed(2), kin: r.kin > 0.4, exchanges: r.exchanges, conflicts: r.conflicts });
      }
    }
  ...
  life(sim, a) {
    ...
    return {
      ...
      name: a.name,
      alive: a.alive,
      age: +a.ageAt(a.alive ? sim.world.tick : a.deathTick).toFixed(1),
      role: a.role,
      titles: a.titles,
      traits: a.traits,
      ...
      cause: a.deathCause,
      milestones: evs.filter((e) => e.landmark || e.kind === 'invention' || e.kind === 'bond' || e.kind === 'death').slice(-6).map((e) => ({ day: Math.floor(e.tick / 24) + 1, text: e.text })),
    };
  }

  /** Plain-language history. Written from the numbers, not from a template bank. */
  narrate(sim, window, prev, last) {
    ...
    lines.push(`${sim.lang.name} stands at ${pop} living ${pop === 1 ? 'soul' : 'souls'} on day ${sim.world.dayNumber} of year ${sim.world.year}, in ${this.era.name}.`);
    if (this.reportCount > 1) {
      lines.push(`Across the last ${days} day${days === 1 ? '' : 's'} the population ${dPop > 0 ? `grew by ${dPop}` : dPop < 0 ? `fell by ${-dPop}` : 'held steady'}, with ${window.filter((e) => e.kind === 'birth').length} born and ${window.filter((e) => e.kind === 'death').length} lost.`);
    }
    const invs = window.filter((e) => e.kind === 'invention');
    if (invs.length) {
      const adv = invs.filter((e) => e.advance);
      lines.push(`They tried ${sim.ont.attempts} things in all and kept ${sim.ont.concepts.size}; ${invs.length} new ${invs.length === 1 ? 'object' : 'objects'} appeared this stretch${adv.length ? `, of which ${adv.length} bettered anything they had before` : ', though none improved on what they already owned'}.`);
    ...
    if (hunger > 0.6) lines.push(`Hunger is the ruling fact — the average belly sits at ${Math.round(hunger * 100)}% empty and the stores are thin.`);
    else if (hunger < 0.3) lines.push('Food is not a worry at present; the fields and beds are keeping pace.');
    const mood = last.mood ?? 0;
    lines.push(`The common mood is ${mood > 0.25 ? 'good' : mood > 0 ? 'level' : mood > -0.25 ? 'strained' : 'grim'} (${mood.toFixed(2)}), and inequality of goods sits at ${(last.gini ?? 0).toFixed(2)}.`);
    ...
    if (conflicts || kindness) lines.push(`There were ${kindness} acts of open kindness and ${conflicts} of harm; the people are ${kindness >= conflicts ? 'still holding together' : 'beginning to turn on each other'}.`);
    const laws = sim.norms.filter((n) => n.strength > 0.5);
    if (laws.length) lines.push(`They now hold ${laws.length} shared rule${laws.length === 1 ? '' : 's'}: ${laws.map((n) => n.label).join('; ')}.`);
    const lost = sim.lostKnowledge.filter((l) => l.tick > this.lastReportTick);
    if (lost.length) lines.push(`${lost.length} thing${lost.length === 1 ? ' was' : 's were'} forgotten when the only person who knew died: ${lost.slice(0, 8).map((l) => l.word).join(', ')}${lost.length > 8 ? `, and ${lost.length - 8} more no one can name` : ''}.`);
    return lines;
  }
}
